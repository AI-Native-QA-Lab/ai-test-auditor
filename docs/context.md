...
## Purpose and v1.2.1 baseline

AI Test Auditor is a deterministic, source-only static analyzer for ineffective JavaScript and TypeScript tests. v1.2.1 is the current stable release: it includes the advisory-workflow CI fix, while the explicit gate still blocks only deterministic `FAKE` findings and never treats an unflagged test as strong.
