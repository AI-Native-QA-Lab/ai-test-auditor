...
v1.1.1 is the current stable release. It is a deterministic, source-only audit of JavaScript and TypeScript test source, with an explicit opt-in `FAKE`-only policy gate. A release number is not proof that every planned goal for that version was completed; a passing audit or gate is not proof that tests are strong.
...
`v1.2` continues the unfinished v1.1 foundation work because `v1.1`/`v1.1.1` version numbers are already used. It also enhances existing audits and report capabilities. It must remain source-only: no runtime execution, network use, semantic inference, or `STRONG` conclusion from an unflagged test.
